...
- **captain [ticket-id…]** — Fleet manager for ticket-working agents (**Claude Code only** — uses background agent threads). Routes Remundo tickets to their repo, spawns one worker per ticket in its own worktree to investigate, implement, test (evidence contract + roadmap in `captain/testing.md`), and ship via **pr**; PR-comment actions gate through you. Bare `/captain` = fleet status + housekeeping. Depends on **pr**, **pr-comments**.
